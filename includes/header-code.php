...
<div id="wrap" class="<?php echo (is_user_logged_in()?"wrap-login":"wrap-not-login").($header_fixed == "on"?" fixed-enabled":"")?>">
			...
			<div class="discy-container the-main-container">
				...
				<div class="right-header float_r">
					<?php if (!is_user_logged_in()) {?>
						<a class="sign-in-lock mob-sign-<?php echo ($mobile_sign == "signup"?"up":"in").apply_filters('wpqa_pop_up_class','').apply_filters('wpqa_pop_up_class_'.($mobile_sign == "signup"?"signup":"login"),'')?>" href="<?php echo (has_wpqa()?($mobile_sign == "signup"?wpqa_signup_permalink():wpqa_login_permalink()):"#")?>" data-toggle="modal"><i class="icon-lock"></i></a>
										...
										if ($is_super_admin || (is_array($moderator_categories) && !empty($moderator_categories))) {
											if ($is_super_admin || in_array(0,$moderator_categories)) {
												...
												if ($moderator_categories != "") {
													$num_pending_questions = wpqa_count_posts_by_user(0,"question","draft",(is_array("q-0",$moderator_categories)?0:$moderator_categories));
													$num_pending_posts = wpqa_count_posts_by_user(0,"post","draft",(is_array("p-0",$moderator_categories)?0:$moderator_categories));
													$num_pending = $num_pending_questions+$num_pending_posts;
												}
											}
										}
	...
	<div class="discy-content main-content">
