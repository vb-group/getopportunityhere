<?php get_header();
	...
	if (!$is_super_admin && is_array($blocked_users) && in_array($user_id,$blocked_users)) {
		echo '<div class="alert-message error"><i class="icon-cancel"></i><p>'.esc_html__("Sorry, you blocked from this group.","wpqa").'</p></div>';
	}else {
		...
		<section class="content_group row">
			<div class="col col12">
				...
				<?php }?>
				<div>
					<?php if ($group_privacy == "public" || ($group_privacy == "private" && is_array($group_users_array) && in_array($user_id,$group_users_array))) :
						...
						if (is_array($sticky_posts) && !empty($sticky_posts)) {
							...
							$wpqa_query = new WP_Query($sticky_data);
						...
						if ($wpqa_query->have_posts()) :
							...
							echo '<div class="alert-message warning"><i class="icon-flag"></i><p>'.esc_html__("There are no posts yet.","wpqa").'</p></div>';
						endif;
					else:
						echo '<div class="alert-message warning"><i class="icon-flag"></i><p>'.esc_html__("Sorry, this is a private group.","wpqa").'</p></div>';
					endif;?>
				</div>
