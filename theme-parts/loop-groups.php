...
if ($wpqa_query->have_posts()) :?>
			...
			echo '<article class="'.join(' ',get_post_class("col col6",$post_id)).'">
				<div class="group-item">
					...
					<div class="group_statistics">
						<div class="tooltip-n" title="'.wpqa_count_number($group_posts).' '._n("Post","Posts",$group_posts,"wpqa").'"><i class="icon-trophy"></i></div>
						<div class="tooltip-n" title="'.wpqa_count_number($group_users).' '._n("User","Users",$group_users,"wpqa").'"><i class="icon-users"></i></div>
						<div class="tooltip-n" title="'.$human_time_diff." ".esc_html__("ago","wpqa").'"><i class="icon-lifebuoy"></i></div>
					</div>
				</div>
			</article>';
		endwhile;?>
	...
	echo '<div class="alert-message warning"><i class="icon-flag"></i><p>'.esc_html__("There are no groups yet.","wpqa").'</p></div>';
endif;?>
