...
define("discy_old_options","discy_old_options");
...
if (is_admin()) {
	...
	function discy_screen_belongs_to_theme_options() {
		...
		if ( ( $pagenow == 'edit.php' && array_key_exists( 'post_type', $_GET ) && $_GET['post_type'] == 'group' ) || ( $pagenow == 'edit.php' && array_key_exists( 'post_type', $_GET ) && $_GET['post_type'] == 'posts' ) || ( $pagenow == 'edit.php' && array_key_exists( 'post_type', $_GET ) && $_GET['post_type'] == 'statement' ) || ( ($pagenow == 'themes.php' || $pagenow == 'admin.php') && array_key_exists( 'page', $_GET ) && $_GET['page'] == 'options' ) || ( ($pagenow == 'themes.php' || $pagenow == 'admin.php') && array_key_exists( 'page', $_GET ) && $_GET['page'] == 'registration' ) || ( $pagenow == 'edit.php' && array_key_exists( 'post_type', $_GET ) && $_GET['post_type'] == 'request' ) || ( $pagenow == 'edit.php' && array_key_exists( 'post_type', $_GET ) && $_GET['post_type'] == 'notification' ) || ( $pagenow == 'edit.php' && array_key_exists( 'post_type', $_GET ) && $_GET['post_type'] == 'activity' ) || ( $pagenow == 'edit.php' && array_key_exists( 'post_type', $_GET ) && $_GET['post_type'] == 'report' ) || in_array( $pagenow, $white_label ) || $show_admin_scripts == true ) {
			return true;
		}
		return false;
	}
...
function discy_comment($comment,$args,$depth,$answer = "",$owner = "",$k_ad = "",$best_answer = "",$answer_args = array()) {
    ...
    $privacy_credential = (has_wpqa() && wpqa_plugin_version >= 4.2?($comment_user_id > 0?wpqa_check_user_privacy($author_id,"credential"):""):"");
                ...
                <div class="text">
                	...
                	<div<?php echo (isset($its_question) && $its_question == "question" && is_single()?" itemprop='text'":"")?>>
                		<?php if (isset($answer_args["comment_with_title"]) || isset($answer_args["comment_read_more"]) || isset($args["comment_read_more"])) {
			            	echo '<p class="less_answer_text">'.wp_html_excerpt($comment->comment_content,apply_filters('discy_answer_number',300),'<a class="post-read-more comment-read-more read_more_answer" href="'.get_permalink($post_id).'#comment-'.esc_attr($comment_id).'" rel="bookmark" title="'.esc_attr__('Read more','discy').'">'.esc_html__('Read more','discy').'</a>').'</p>
			            	<div class="wpqa_hide full_answer_text">';
                				comment_text($comment_id);
			            		do_action("discy_action_comment_after_comment_text",$comment,$post_type);
                				echo '<a class="read_less_answer" href="#">'.esc_html__("See less","discy").'</a>
                			</div>';
			            }else {
			            	comment_text($comment_id);
			            	do_action("discy_action_comment_after_comment_text",$comment,$post_type);
                	...
                	if (has_wpqa() && isset($args["comment_type"]) && $args["comment_type"] == "comment_group") {
                		$activate_edit = wpqa_group_edit_comments($post_id,$is_super_admin,$can_edit_comment,$comment_user_id,$user_get_current_user_id);
						$activate_delete = wpqa_group_delete_comments($post_id,$is_super_admin,$can_delete_comment,$comment_user_id,$user_get_current_user_id);
                	}
		...
		if (($custom_permission == "on" && is_user_logged_in() && !is_super_admin($user_id) && empty($roles["without_ads"])) || ($custom_permission == "on" && !is_user_logged_in())) {
			if ($class == "discy-ad-header") {
				...
				if ($subscriptions_payment == 'on' && has_wpqa()) {
					$show_alert = '<div class="alert-message error"><i class="icon-cancel"></i><p>'.esc_html__("Do you need to remove the ads?","discy").' '.wpqa_paid_subscriptions().'</p></div>';
				}
			}
		}else {
			$show_ads = false;
			...
			if ((isset($discy_home_tabs[$key]["sort"]) && isset($discy_home_tabs[$key]["value"]) && $discy_home_tabs[$key]["value"] != "" && $discy_home_tabs[$key]["value"] != "0") || (isset($discy_home_tabs[$key]["value"]) && isset($discy_home_tabs[$key]["cat"]))) {
				...
				if (isset($last_url) && $last_url != "") {
					...
					if ($list_child == "li") {?>
						...
						<option<?php echo (isset($first_one) && $first_one != "" && $first_one == $last_url?" selected='selected'":"")?> value="<?php echo esc_url($get_url)?>">
					<?php }
							if (isset($last_url) && isset($discy_home_tabs[$key]["sort"])) {
