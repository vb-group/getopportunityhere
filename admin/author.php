...
function discy_admin_author($user_id = "") {
	...
	if (is_super_admin($get_current_user_id)) {
		...
		if ($active_activity_log == "on") {
			$_whats_types = get_user_meta($user_id,$user_id."_activities",true);
			if (isset($_whats_types) && $_whats_types > 0) {
				$html_content = '<a class="button delete-rows" data-history="activity" data-user="'.$user_id.'" href="'.admin_url("user-edit.php?user_id=".$user_id."&rows=delete&delete_type=activities").'">'.esc_html__("Delete all the activities history","discy").'</a>';
				$options[] = array(
					'name' => $html_content,
					'type' => 'info'
				);
		...
		if ($active_notifications == "on") {
			$_whats_types = get_user_meta($user_id,$user_id."_notifications",true);
			if (isset($_whats_types) && $_whats_types > 0) {
				$html_content = '<a class="button delete-rows" data-history="notification" data-user="'.$user_id.'" href="'.admin_url("user-edit.php?user_id=".$user_id."&rows=delete&delete_type=notifications").'">'.esc_html__("Delete all the notifications history","discy").'</a>';
				$options[] = array(
					'name' => $html_content,
					'type' => 'info'
				);
